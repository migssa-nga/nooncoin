...
*Setup instructions for a Gitian build of Nooncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nooncoin
...
- [Building Nooncoin Core](#building-nooncoin-core)
...
Clone the git repositories for nooncoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nooncoin-project/nooncoin
...
in the nooncoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Nooncoin Core
----------------

To build Nooncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the nooncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nooncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/nooncoin-project/nooncoin
...
For example:
```bash
URL=https://github.com/thrasher-/nooncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit nooncoin=${COMMIT} --url nooncoin=${URL} ../nooncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nooncoin=${COMMIT} --url nooncoin=${URL} ../nooncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nooncoin=${COMMIT} --url nooncoin=${URL} ../nooncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the nooncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../nooncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/nooncoin
SIGPATH=/some/root/path/nooncoin-detached-sigs

./bin/gbuild --url nooncoin=${BTCPATH},signature=${SIGPATH} ../nooncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nooncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nooncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/nooncoin-osx-build.assert
```
...
[nooncoin-project/gitian.sigs.ltc](https://github.com/nooncoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
