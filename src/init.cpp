...
bool static Bind(CConnman& connman, const CService &addr, unsigned int flags) {
    if (!(flags & BF_EXPLICIT) && IsLimited(addr))
        return false;
    std::string strError;
    if (!connman.BindListenPort(addr, strError, (flags & BF_WHITNOONST) != 0)) {
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-whitelistrelay", strprintf(_("Accept relayed transactions received from whitelisted peers even when not relaying transactions (default: %d)"), DEFAULT_WHITNOONSTRELAY));
    strUsage += HelpMessageOpt("-whitelistforcerelay", strprintf(_("Force relay of transactions from whitelisted peers even if they violate local relay policy (default: %d)"), DEFAULT_WHITNOONSTFORCERELAY));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nooncoin/nooncoin>";
    const std::string URL_WEBSITE = "<https://nooncoin.com>";
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-whitelistforcerelay", DEFAULT_WHITNOONSTFORCERELAY)) {
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (fListen) {
        ...
        if (mapMultiArgs.count("-whitebind")) {
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs.at("-whitebind")) {
                ...
                fBound |= Bind(connman, addrBind, (BF_EXPLICIT | BF_REPORT_ERROR | BF_WHITNOONST));
            }
        }
    ...
    connman.AddNode("seed1.nooncurrency.com");
    connman.AddNode("explorer.nooncurrency.com");
    connman.AddNode("pool.nooncurrency.com");
    connman.AddNode("api.nooncurrency.com");
    connman.AddNode("wallet.nooncurrency.com");
    connman.AddNode("node.nooncurrency.com");
    connman.AddNode("electrum.nooncurrency.com");
